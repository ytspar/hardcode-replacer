import { sampleLines, colorMatches, variables, type ColorMatch } from './data';
import { wait, isReducedMotion } from './animation';
...
let colorElements: ColorElement[] = [];
...
export function mountDemo(container: HTMLElement): void {
  ...
  for (const [color, label] of legendItems) {
    ...
    if (color === 'var(--green)') dot.style.background = '#22c55e';
    else if (color === 'var(--amber)') dot.style.background = '#f59e0b';
    else if (color === 'var(--red)') dot.style.background = '#ef4444';
...
function createColorElement(text: string, match: ColorMatch): HTMLElement {
  ...
  wrapper.setAttribute('aria-label', `Color ${text}, ${match.match} match${match.replacement ? `, replaceable with ${match.replacement}` : ''}`);
  ...
  const colorEl: ColorElement = { wrapper, original, replacement, match, transformed: false };
  colorElements.push(colorEl);

  // Click/keyboard handler
  const toggle = () => {
    if (match.match === 'unmatched' || match.match === 'effect') return;
...
async function transformColor(el: ColorElement): Promise<void> {
  if (el.transformed || !el.match.replacement) return;
  ...
  if (!isReducedMotion()) {
    ...
    void el.replacement.offsetWidth;
    await wait(30);
...
async function replaceAll(): Promise<void> {
  const exactMatches = colorElements.filter(
    el => (el.match.match === 'exact' || el.match.match === 'close') && !el.transformed
  );
...
function showConnection(colorEl: ColorElement): void {
  if (!connectionsSvg || !colorEl.match.varName) return;

  hideConnections();

  // Find matching variable row
  const varRow = document.querySelector(`[data-var="${colorEl.match.varName}"]`);
  if (!varRow) return;
  ...
  line.setAttribute('y1', String(srcRect.top + srcRect.height / 2 - svgRect.top));
  line.setAttribute('x2', String(dstRect.left - svgRect.left));
  line.setAttribute('y2', String(dstRect.top + dstRect.height / 2 - svgRect.top));
...
function hideConnections(): void {
  document.querySelectorAll('.demo-var.highlighted').forEach(el => el.classList.remove('highlighted'));
