...
import { type HeroColor, heroColors } from "./data";
...
export function mountHero(container: HTMLElement): void {
  ...
  const bottomWord = createTitleWord(
    "Replacer",
    "Variables",
    "hero-title-bottom"
  );
  ...
  tagline1.textContent =
    "A CLI tool for finding hardcoded colors and replacing them with variables.";
  const tagline2 = document.createElement("div");
  tagline2.className = "hero-accent-text";
  tagline2.textContent =
    "Especially useful for cleaning up Claude Code generated interfaces.";
...
function transformTitleWord(word: TitleWord): void {
  if (word.transformed) {
    return;
  }
...
function createRow(color: HeroColor): HeroRow {
  ...
  wrapper.setAttribute(
    "aria-label",
    `${color.hex} transforms to ${color.varRef}`
  );
...
function transformRow(row: HeroRow): void {
  if (row.transformed) {
    return;
  }
